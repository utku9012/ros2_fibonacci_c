...
class FibonacciActionClient
    : public rclcpp::Node { // rclcpp:Node'dan türetilmiş bir class
                            // oluşturuyoruz
public:
  ...
  explicit FibonacciActionClient(const rclcpp::NodeOptions &options)
    ...
    // ros timer send goal çağıracak
  }

  void send_goal() { // send_goal fonksiyonu
    using namespace std::placeholders;

    this->timer_->cancel(); // 1 kere çağırılması için iptal ediyoruz

    if (!this->client_ptr_->wait_for_action_server()) {
      RCLCPP_ERROR(this->get_logger(),
                   "Action server not available after waiting");
      rclcpp::shutdown();
    } // action server bekleniyor

    auto goal_msg = Fibonacci::Goal(); // Fibonacci::Goal
    goal_msg.order = 20; // ilk 20 fibonacci sayısı

    RCLCPP_INFO(this->get_logger(), "Sending goal");

    auto send_goal_options =
        rclcpp_action::Client<Fibonacci>::SendGoalOptions();
    send_goal_options.goal_response_callback = // response
    ...
    send_goal_options.feedback_callback = // feedback
    ...
    send_goal_options.result_callback = // result
